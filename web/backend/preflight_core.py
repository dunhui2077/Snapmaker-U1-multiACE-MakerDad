...
def parse_meta(pp, line_iter):
    ...
    need from the file metadata. Works on any iterable of lines, so both the
    backend and the browser worker can pass an open file handle (memory-friendly
    for huge files).
